...
export default function PrimaryButton({ children, className, }: PrimaryButtonProps) {
    return (
        <button className={`w-fit flex items-center cursor-pointer relative group ${className}`}>
            <div className="border-[1px] border-neutral-300 group-hover:border-transparent duration-300 py-[14px] pl-6 pr-8 rounded-[44px]">
                <Paragraph variant="para-10" className="text-neutral-50 group-hover:text-primary-500 duration-300 relative left-0 z-10">
                    {children}
                </Paragraph>
            </div>
            <div className="w-[52px] group-hover:w-[calc(100%_+_33px)] duration-500 h-[52px] border-[1px] border-secondary-500 rounded-full bg-secondary-500 flex items-center justify-center absolute right-[-35px]">
